        ...
        with build_model_context(**build_model_context_args):
            if args.base_model == "vanilla":
                ...
                raise NotImplementedError(
                    "Models other than vanilla GPT models are not supported yet."
                )
...
def _get_packed_seq_params(tokens, labels, loss_mask, attention_mask, position_ids):
    ...
    if not (
        args.reset_position_ids and args.reset_attention_mask
    ):  # we must turn on both to use packed sequence
    ...
    if args.cut_off_varlen_to_seqlen:
        seqlen_multiples = torch.arange(
            args.seq_length,
            tokens.shape[0] + args.seq_length,
            args.seq_length,
        ...
        raise NotImplementedError(
            "Packing with cross-sequence attention is not yet supported as the dataset blending & shuffling will break the consecutive sequence assumption."
        )
    ...
    cu_seqlens = torch.unique(
        torch.cat((zero_start, seq_ends, cu_end))
    )  # zero_start and the cu_end can happen to be the start/end of a document
...
def get_batch(data_iterator):
    ...
    tokens, labels, loss_mask, attention_mask, position_ids, packed_seq_params = (
        _get_packed_seq_params(tokens, labels, loss_mask, attention_mask, position_ids)
    )
...
def loss_func(loss_mask: torch.Tensor, model: GPTModel, output_tensor: torch.Tensor):
    ...
    if args.model_running_mode == "difflm-noshift":
        ...
        logging_dict = {'lm loss': (reporting_loss[0], reporting_loss[1])}
    elif args.model_running_mode == "test-forward":
        logging_dict = {'lm loss': (reporting_loss[0], reporting_loss[1])}
    else:
        raise NotImplementedError("not supported yet, need implementation.")

    return (loss[0] * args.context_parallel_size, local_num_tokens, logging_dict)
...
def forward_step(data_iterator, model: GPTModel):
    ...
    with stimer:
        output_tensor, difflm_mask = model(

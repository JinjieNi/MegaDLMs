...
def validate_args(args, defaults={}):
    ...
    if args.encoder_pipeline_model_parallel_size == 0 and args.num_experts == 0:
        ...
        ), f"If non-MOE encoder shares first decoder pipeline rank it must have the same TP as the decoder. But got encoder_tensor_model_parallel_size: {args.encoder_tensor_model_parallel_size} and tensor_model_parallel_size: {args.tensor_model_parallel_size}"
    ...
    if (
        ...
        and args.encoder_tensor_model_parallel_size == 0
    ):
        args.encoder_tensor_model_parallel_size = args.tensor_model_parallel_size
    ...
    if getattr(args, "use_torch_fsdp2", False):
        assert get_torch_version() >= PkgVersion(
            "2.4"
...
def _add_network_size_args(parser):
    ...
    group.add_argument(
        ...
        help='Attention backend to use (flash,fused,unfused,local,auto,custom). Defaults to auto',
    )
    group.add_argument(
        '--kv-channels',
        type=int,
    ...
    group.add_argument(
        ...
        choices=[
            ...
            'rope',
            'none',
            'rope_2d_position_step',
            'rope_position_learned_step',
            'rope_position_sinusoid_step',
        ],
    ...
    group.add_argument(
        '--use-rope-scaling', action='store_true', help='Apply rope scaling as used in llama3.1'
    )
    group.add_argument(
...
def _add_logging_args(parser):
    ...
    group.add_argument('--wandb-run-id', type=str, default=None, help='The wandb run id.')
    group.add_argument(
        '--wandb-save-dir', type=str, default='', help='Path to save the wandb results locally.'
    )
    group.add_argument('--wandb-mode', type=str, default='online', help='Mode to run wandb in.')
    group.add_argument(
        '--wandb-tags', type=str, default=None, help='Tags to add to the wandb experiment.'
    )
    group.add_argument(
        '--wandb-group', type=str, default=None, help='Group to add to the wandb experiment.'
    )
    group.add_argument(
        '--wandb-notes', type=str, default=None, help='Notes to add to the wandb experiment.'
    )
    group.add_argument('--wandb-resume', type=str, default='auto', help='Resume policy for wandb.')
    group.add_argument(
        '--wandb-resume-from', type=str, default=None, help='Resume from a specific wandb run.'
    )
...
def _add_training_args(parser):
    ...
    group.add_argument(
        '--build-data-cache-only', action='store_true', help='Whether to build the data cache only.'
    )
...
def _add_moe_args(parser):
    ...
    group.add_argument(
        ...
        choices=[
            ...
            'sinkhorn',
            'none',
            'softmax_expert_choice',
            'sinkhorn_expert_choice',
        ],
        default='aux_loss',
    ...
    group.add_argument(
        '--moe-router-topk-limited-devices',
        type=int,
        default=None,

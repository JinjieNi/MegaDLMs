...
class ForwardStep:
    ...
    def __call__(self, tokens, position_ids, attention_mask, recv_buffer_seq_length=None):
        ...
        if self.pipeline_size_larger_than_one and self.pipelining_batch_x_seqlen != -1:
            ...
            if current_batch_x_seqlen >= self.pipelining_batch_x_seqlen:
                micro_batch_size = max(1, self.pipelining_batch_x_seqlen // seq_len)
                return self._with_pipelining_forward_step(
                    tokens,
                    position_ids,
                    attention_mask,
                    micro_batch_size,
        ...
        return self._no_pipelining_forward_step(
    ...
    def _with_pipelining_forward_step(
        self, tokens, position_ids, attention_mask, micro_batch_size, recv_buffer_seq_length=None
    ):
        """No interleaving is supported."""
        batch_size = tokens.size(0)
        sequence_length = (
            tokens.size(1) if recv_buffer_seq_length is None else recv_buffer_seq_length
        )
        ...
        for micro_batch_index in range(num_micro_batches):
            ...
            if this_micro_batch_size != micro_batch_size:
                recv_buffer = None
            output = self._forward_step_helper(

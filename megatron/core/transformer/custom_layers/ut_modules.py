...
def depthwise_routing(
    self, hidden_states: Tensor, router_layer: nn.Parameter, mask: Tensor = None
) -> Tuple[Tensor, Tensor]:
    """
    ...
    router_logits = F.softmax(router_logits, dim=-1).to(
        original_dtype
    )  # It's important to keep the fp32 for softmax input. See the Figure 15 of the switch transformer paper for details.
...
def adacom_layer_forward(
    ...
    layer_properties = [
        ...
        'initial layer',
        'last layer',
        'global recurrented first layer',
    ]
    ...
    if self.router_layer_i is not None and l_no == 0 and token_states_last is None:
        ...
        assert (
            router_logits_last is None
        ), "router_logits_last must be None when using the initial router layer"
        router_logits_i, router_decisions_i = self.depthwise_routing(
            hidden_states, self.router_layer_i
        )
    ...
    if -2 in token_states_last:  # if the last router is from the global router
        ...
        assert (
            0 not in token_states_last and 2 not in token_states_last
        ), "The tokens routed from the global router must be either global recurrent, model halted, or skipped to the next layer"
    ...
    assert (
        -2 not in token_states_last
    ), "The token_states_last must not contain global recurrent states"
    assert (
        -1 not in token_states_last
    ), "The token_states_last must not contain recurrent states"  # this state only happens in the loop
    ...
    while max_recurrent_steps > -1:
        ...
        if max_recurrent_steps_ori > max_recurrent_steps:
            hidden_states = hidden_states * (1 - recurrent_mask) + hidden_states * (
                router_logits[:, :, 0].squeeze(-1) * recurrent_mask
            )
        ...
        if max_recurrent_steps_ori == max_recurrent_steps:
            ...
            router_logits, router_decisions = self.depthwise_routing(
                hidden_states, self.router_layers[l_no], router_mask
            )
        else:
        ...
        recurrent_mask[router_decisions != 0] = (
            0  # here the router_decisions != 0 tokens is a subset of the router mask
        )
...
def act_halt_function(self, state):
    ...
    p = ut_router_activation(
        ut_router_layer(ut_router_layer_norm(state.to(torch.float32)))[
            0
        ]  # using ColumnParallelLinear, so we need to take the first element
    ).squeeze(-1)
    return p.to(original_type)


def should_continue(halting_probability, n_updates, threshold, act_max_steps):
    condition = torch.logical_and(halting_probability < threshold, n_updates < act_max_steps)
...
def _ut_act_transformer_forward(
    ...
    if self.config.position_embedding_type == 'rope_2d_position_step':
        ...
        if inference_params is not None:
            ...
            position_ids_1 = (
                torch.arange(inf_max_seq_length, device=hidden_states.device)
                .unsqueeze(0)
                .expand(b, -1)
            )  # [b, inf_max_seq_length]
            ...
            if not hasattr(inference_params, 'rope2d_nupdates_memory_dict'):
                ...
                inference_params.rope2d_nupdates_memory_dict['rope2d_nupdates_memory_alllayers'] = (
                    rope2d_nupdates_memory
                )
            else:
                rope2d_nupdates_memory = inference_params.rope2d_nupdates_memory_dict[
                    'rope2d_nupdates_memory_alllayers'
                ]
            ...
            position_ids_2 = rope2d_nupdates_memory[:inf_max_seq_length, :].transpose(
                0, 1
            )  # [b, inf_max_seq_length]
        else:
            position_ids_1 = (
                torch.arange(s, device=hidden_states.device).unsqueeze(0).expand(b, -1)
            )  # [b, s]
            position_ids_2 = position_ids_2.transpose(0, 1)  # [b, s]

        position_ids = torch.stack(
            [position_ids_1, position_ids_2], dim=0
        )  # [2, b, s] / [2, b, inf_max_seq_length]
        rotary_pos_cos, rotary_pos_sin = self.rotary_pos_emb(
            hidden_states, position_ids
        )  # [2, s, b, h] / [2, inf_max_seq_length, b, h]
...
def ut_act_forward_original(
    ...
    halting_probability = torch.zeros(
        state.size(0), state.size(1), device=state.device, dtype=state.dtype
    )  # [s, b]
    remainders = torch.zeros(
        state.size(0), state.size(1), device=state.device, dtype=state.dtype
    )  # [s, b]
    n_updates = torch.zeros(
        state.size(0), state.size(1), device=state.device, dtype=torch.int32
    )  # [s, b]
    previous_state = torch.zeros_like(state, device=state.device, dtype=state.dtype)  # [s, b, h]
    step = torch.tensor(
        0, device=state.device, dtype=torch.int32
    )  # it only works for global input step encoding
    ...
    def act_function(
        ...
        update_weights = (p * still_running + remainders * new_halted).unsqueeze(
            -1
        )  # shape: [s, b, 1]
        ...
        transformed_state, _ = (
            _ut_act_transformer_forward(  # TODO Current implementation is not saving compuation/memory. Implement the compuation/memory or memory efficient version, though it could be a bit tricky due to the fact that the active tokens in each sequence are not the same.
                self,
        ...
        if self.config.act_type == "basic":
            ...
            )  # Every step the state is normalize to 1. However, it will decay the previous states (give preference to the latest ones, which might be better). The halted states being intact.
        elif self.config.act_type == "accumulated":
            new_state = (
                transformed_state * update_weights
            ) + previous_state  # it won't decay the previous states. the author actually used the basic one as he claims it performed better in his experiments.
        elif self.config.act_type == "latest":
            new_state = (
                transformed_state * update_weights
            )  # apply the predicted logits to make the halting function differentiable
        else:
...
def sut_act_forward(
    ...
    accumulated_nonhalting_probability = torch.ones(
        state.size(0), state.size(1), device=state.device, dtype=state.dtype
    )  # [s, b]
    accumulated_states = torch.zeros_like(
        state, device=state.device, dtype=state.dtype
    )  # [s, b, h]
    ...
    if self.config.act_loss_type == "kl_geo":
        history_logits = torch.zeros(
            state.size(0), state.size(1), act_max_steps, device=state.device, dtype=state.dtype
        )
    ...
    def act_function(
        ...
        # apply transformation on the state
        transformed_state, _ = (
            _sut_act_transformer_forward(  # TODO Current implementation is not saving compuation/memory. Implement the compuation/memory or memory efficient version, though it could be a bit tricky due to the fact that the active tokens for each sequence of the batchare not the same.
                self,
                hidden_states=transformed_state,
                attention_mask=attention_mask,
        ...
        new_state = torch.where(
            (_active_mask).unsqueeze(-1), state_to_update, previous_state
        )  # [s, b, h]
...
def should_continue_router(still_running, n_updates, act_max_steps):
    condition = torch.logical_and(still_running, n_updates < act_max_steps)
    # Return whether any of the condition is True
    return torch.any(condition)
    ...
    for i in range(len(history_states)):
        for j in range(len(history_states[i])):
            if softmax:
                ...
                history_states[i][j] = torch.einsum(
                    'bs, bsh -> bsh', softmax_logits, torch.stack(history_states[i][j])
                )
            else:
                history_states[i][j] = torch.einsum(
                    'bs, bsh -> bsh',
                    torch.stack(history_logits[i][j]),
                    torch.stack(history_states[i][j]),
                )
...
def ut_router_forward(
    ...
    still_running = torch.ones(
        state.size(0), state.size(1), device=state.device, dtype=torch.bool
    )  # [s, b]
    new_halted = torch.zeros(
        state.size(0), state.size(1), device=state.device, dtype=torch.bool
    )  # [s, b]
    n_updates = torch.zeros(
        state.size(0), state.size(1), device=state.device, dtype=torch.int32
    ...
    def act_function(
        ...
        if not self.config.act_fake_halting:
            ...
            active_mask = still_running0.unsqueeze(
                -1
            )  # [s, b, 1], the newly halted states will not be transformed
            halt_mask = ~active_mask  # [s, b, 1]
        ...
        # apply transformation on the state
        transformed_state, _ = (
            _ut_router_transformer_forward(  # TODO Current implementation is not saving compuation/memory. Implement the compuation/memory or memory efficient version, though it could be a bit tricky due to the fact that the active tokens in each sequence are not the same.
                self,
                hidden_states=state,
                attention_mask=attention_mask,
                context=context,
        ...
        halted_part *= p0.unsqueeze(
            -1
        )  # todo: applying logits to the halted states is not necessary, as did in MoD.
        ...
        if self.config.skip_connection:
            transformed_state = (
                transformed_state + state
            )  # only the non-halted states need a skip connection. May not need as the original transformer block already has a skip connection.
    ...
    with rng_context, fp8_context:
        # Forward pass.
        while should_continue_router(still_running, n_updates, act_max_steps):
            (
                state,
                step,
                p,
...
def ut_router_prerouting_forward(
    ...
    step = torch.tensor(
        0, dtype=n_updates.dtype, device=state.device
    )  # it only works for global input step encoding

    if not self.config.act_type == "latest":
        history_logits = [
            [[] for _ in range(state.size(1))] for _ in range(state.size(0))
        ]  # [s, b]
        history_states = [
            [None for _ in range(state.size(1))] for _ in range(state.size(0))
        ]  # [s, b]
    else:
        history_logits = None
        history_states = None
    ...
    def act_function(
        ...
        p = router_function(
            self.ut_router_activation, self.ut_router_layer, state
        )  # the halt function prediction should be done in float32 to increase stability
        ...
        if not self.config.act_fake_halting:
            ...
            # Only transform non-halted states, this include the newly halted states
            newly_halted = new_halted.unsqueeze(-1)  # [s, b, 1]
            active_mask = still_running.unsqueeze(
                -1
            )  # [s, b, 1], the newly halted states will not be transformed
            halt_mask = ~active_mask  # [s, b, 1]

            # Store transformed states of the newly halted positions
            halted_states = update_halted_states(
                source_states=state, halted_states=halted_states, mask=newly_halted
            )
        ...
        if not self.config.act_fake_halting:
            # Combine transformed active states with halted states
            transformed_state = retrieve_states(
                source_states=transformed_state, halted_states=halted_states, mask=active_mask
            )

        transformed_state = (
            active_mask * transformed_state * (1 - p)
            + new_halted.unsqueeze(-1) * transformed_state * p
        )
    ...
    act_loss = compute_act_loss_router(
        history_logits, self.config.act_loss_type, self.config.act_loss_factor
    )

        ...
        super().__init__(
            {
                ...
                "pooler": {"weight": "pooler.dense.weight", "bias": "pooler.dense.bias"},
                "lm_head": {
                ...
                "binary_head": {"weight": "binary_head.weight", "bias": "binary_head.bias"},
    ...
    def __init__(self, model_type, num_experts, expert_model_parallel_size, num_shared_experts):
        ...
        super().__init__(
            ...
            layer_schema={
                ...
                **{
                    f"mlp_fc1_weight.{expert_idx}": f"mlp.experts.linear_fc1.weight{expert_idx}"
                    for expert_idx in range(num_local_experts)
                },
                **{
                    f"mlp_fc2_weight.{expert_idx}": f"mlp.experts.linear_fc2.weight{expert_idx}"
                    for expert_idx in range(num_local_experts)
                },
                **{
                    f"mlp_fc1_bias.{expert_idx}": f"mlp.experts.linear_fc1.bias{expert_idx}"
                    for expert_idx in range(num_local_experts)
                },
                **{
                    f"mlp_fc2_bias.{expert_idx}": f"mlp.experts.linear_fc2.bias{expert_idx}"
                    for expert_idx in range(num_local_experts)
                },
                ...
                **{
                    f"shared_mlp_fc1_weight.{expert_idx}": f"mlp.shared_experts.{expert_idx}.linear_fc1.weight"
                    for expert_idx in range(num_shared_experts)
                },
                **{
                    f"shared_mlp_fc2_weight.{expert_idx}": f"mlp.shared_experts.{expert_idx}.linear_fc2.weight"
                    for expert_idx in range(num_shared_experts)
                },
                **{
                    f"shared_mlp_fc1_bias.{expert_idx}": f"mlp.shared_experts.{expert_idx}.linear_fc1.bias"
                    for expert_idx in range(num_shared_experts)
                },
                **{
                    f"shared_mlp_fc2_bias.{expert_idx}": f"mlp.shared_experts.{expert_idx}.linear_fc2.bias"
                    for expert_idx in range(num_shared_experts)
                },
            },
        )
...
def get_model_schema(
    ...
    if num_experts is not None and num_experts > 0:
        ...
        return CoreMoETESchema(
            model_type, num_experts, expert_model_parallel_size, num_shared_experts
        )

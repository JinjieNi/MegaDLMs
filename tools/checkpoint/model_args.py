def extra_args_provider_plmt(parser):
    ...
    group.add_argument('--debug', action='store_true', help='Whether to run in debug mode.')
    group.add_argument('--turn-on-debugpy', action='store_true', help='Whether to turn on debugpy.')
    group.add_argument('--detect-anomaly', action='store_true', help='Whether to detect anomaly.')
    group.add_argument(
        '--model-running-mode',
        type=str,
        default="plmt-lnwp",
        choices=["plmt", "plmt-lnwp", "vanilla"],
    ...
    group.add_argument(
        '--use-embedding-hidden-weight-fusion-bias',
        action='store_true',
    ...
    group.add_argument(
        '--max-l', type=int, default=10, help='The maximum latent span corruption length.'
    )
    group.add_argument(
        '--sigma-p',
        type=float,
    ...
    group.add_argument(
        '--no-mask-latent-span-loss',
        action='store_false',
        dest='mask_latent_span_loss',
    ...
    group.add_argument(
        '--no-prefix-tokens-without-grad',
        action='store_false',
        dest='prefix_tokens_without_grad',
    ...
    group.add_argument(
        '--no-mixed-depth-supervision',
        action='store_false',
        dest='mixed_depth_supervision',
...
def extra_args_provider_difflm(parser):
    ...
    group.add_argument(
        ...
        type=str,
        default="difflm",
        choices=["difflm", "difflm-conditional", "vanilla", "test-forward"],
    ...
    group.add_argument(
        '--mask-token',
        type=int,
        default=151656,  # 151656 is the <|video_pad|> token of Qwen 2.5

...
def add_arguments(parser):
    ...
    group.add_argument(
        '--sequence-parallel', action='store_true', help='Whether to use sequence parallel.'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--release', action='store_true', help='Whether to the checkpoint is a release checkpoint'
    )
...
def save_checkpoint(queue, args):
    ...
    margs.sequence_parallel = (
        md.checkpoint_args.sequence_parallel if "sequence_parallel" in md.checkpoint_args else False
    )
    margs.apply_query_key_layer_scaling = (
        md.checkpoint_args.apply_query_key_layer_scaling
        if "apply_query_key_layer_scaling" in md.checkpoint_args
        else False
    )
    ...
    models = [
        [
            [None for _ in range(args.target_tensor_parallel_size)]
    ...
    def get_local_model(pp_rank, ep_rank, tp_rank):
        if models[pp_rank][ep_rank][tp_rank] is None:
            ...
            models[pp_rank][ep_rank][tp_rank] = model_provider(pre_process, post_process).to(
                md.params_dtype
            )
        ...
        if weight.dim() == 3:
            ...
            if parallel_mode == "column":
                weight = weight.reshape(
                    ep_size, num_experts // ep_size, tp_size, out_features // tp_size, in_features
                )
                weight = weight.permute(0, 2, 1, 3, 4)
            else:
                weight = weight.reshape(
                    ep_size, num_experts // ep_size, out_features, tp_size, in_features // tp_size
                )
            ...
            if parallel_mode == "column":
                ...
                weight = weight.reshape(out_features, tp_size, in_features // tp_size).permute(
                    1, 0, 2
                )
    ...
    def chunk_bias(bias, parallel_mode, tp_size=1, ep_size=1):
        ...
        if bias.dim() == 2:
            ...
            if parallel_mode == 'column':
                bias = bias.reshape(
                    ep_size, num_experts // ep_size, tp_size, hidden_size // tp_size
                )
                bias = bias.permute(0, 2, 1, 3)  # (ep_size, tp_size, local_eps, hidden_size)
            else:
                bias = bias.reshape(
                    ep_size, num_experts // ep_size, hidden_size
                )  # (ep_size, local_eps, hidden_size)
            return bias
        else:
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for layer_id in range(schema.get_num_layers(models[pp_rank][0][0])):
            ...
            qkv_weight = chunk_weight(
                msg.pop("qkv weight"), "column", args.target_tensor_parallel_size
            )
            dense_weight = chunk_weight(
                msg.pop("dense weight"), "row", args.target_tensor_parallel_size
            )
            ...
            if md.qkv_bias:
                qkv_bias = chunk_bias(
                    msg.pop("qkv bias"), 'column', args.target_tensor_parallel_size
                )
            if md.linear_bias:
                ...
                for tp_rank in range(args.target_tensor_parallel_size):
                    ...
                    if md.linear_bias:
                        ...
                        if margs.num_experts:
                            ...
                            params_dict.update(
                                {"mlp_fc1_bias": mlp_l0_bias[tp_rank], "mlp_fc2_bias": mlp_l1_bias}
                            )
                    if margs.num_experts:
                    ...
                    if md.norm_has_bias:
                        params_dict.update({"q_norm_bias": q_norm_bias, "k_norm_bias": k_norm_bias})
        ...
        if pp_rank == args.target_pipeline_parallel_size - 1:
            ...
            if md.output_layer:
                ...
                output_layer_weight = torch.chunk(
                    output_layer_weight, args.target_tensor_parallel_size, dim=0
                )
                for eptp_rank, model in enumerate(pp_local_models):
                    tp_rank = eptp_rank % args.target_tensor_parallel_size
            ...
            if (
                ...
                if not hasattr(pp_local_models[0], 'embedding_hidden_fusion_layer'):
                    print(
                        "ERROR: got an lnwp embedding hidden fusion layer, but model does not have one"
                    )
                    exit(1)
                embedding_hidden_fusion_layer_weight = msg.pop("weight")
                embedding_hidden_fusion_layer_weight = chunk_weight(
                    embedding_hidden_fusion_layer_weight,
                    "row",
                    args.target_tensor_parallel_size,
                    args.target_expert_parallel_size,
                )
                ...
                if (
                    ...
                    embedding_hidden_fusion_layer_bias = chunk_weight(
                        embedding_hidden_fusion_layer_bias,
                        "row",
                        args.target_tensor_parallel_size,
                        args.target_expert_parallel_size,
                    )
            ...
            if (
                margs.model_running_mode == "difflm-noshift-uniform"
                and margs.difflm_use_time_embedding
            ):

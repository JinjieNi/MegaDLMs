...
from megatron.training import get_args, get_model, get_retro_args, get_tokenizer, print_rank_0
from megatron.training.arguments import core_transformer_config_from_args
from megatron.training.checkpointing import load_checkpoint
from megatron.training.initialize import initialize_megatron
from tools.retro.sft.dataset_conv import preprocess, reformat_prompt, reformat_prompt_short
...
def pad_neighbours_for_query_only(args, nb_tokens, pad_id, ft_neighbours):
    ...
    if len(neighbours_tokens) < args.retro_num_neighbors:
        assert ValueError(
            "neighbours are not enough, add empty ones and create mask for those empty ones"
        )
...
def add_text_generate_args(parser):
    ...
    group.add_argument(
        "--out-seq-length", type=int, default=256, help='Size of the output generated text.'
    )
    group.add_argument(
        "--sample-input-file",
        type=str,
    ...
    group.add_argument('--length-penalty', type=float, default=1.0, help='length penalty')
    group.add_argument('--gen-start-idx', type=int, default=0, help='project size for adapters')
    group.add_argument('--num-gen', type=int, default=-1, help='project size for adapters')
    group.add_argument('--ckpt-step', type=int, default=None, help='setting ckpt step manually')
    group.add_argument("--short-format", action='store_true', help='Use short format QA')
    group.add_argument(
    ...
    group.add_argument('--template-id', type=int, default=0, help='template id for generation,')
    return parser
        ...
        if torch.distributed.get_rank() == 0:
            ...
            for _ in range(args.global_batch_size):
                ...
                if True:
                    ...
                    neighbours_array = pad_neighbours_for_query_only(
                        args,
                        [tokenizer.tokenize(neighbour) for neighbour in neighbours],
                        tokenizer.eod,
                        args.ft_neighbours,
                    )
                    ...
                    if args.short_format:
                        input_tokens = reformat_prompt_short(
                            query,
                            neighbours,
                            args.task,
                            args.ft_neighbours,
            ...
            resp_sentences, resp_sentences_seg, scores, tokens = retro_generate_and_post_process(
                model,
                prompts=sentences,

...
def convert_wqkv(llama_mega, layer_idx=0, n_heads=32, n_heads_kv=8):
    ...
    assert qkv_w.shape == (
        ...
        h // ng + 2 * hn,
        h,
    ), f"expected shape: {(ng, h//ng + 2*hn, h)}, got shape: {qkv_w.shape}"
...
def convert_ffn_shared_experts_separate(s_ep_id, llama_mega, layer_idx=0, n_dense=11008):
    mega_ffn = llama_mega[
        f'decoder.layers.{layer_idx}.mlp.shared_experts.{s_ep_id}.linear_fc1.weight'
    ]
    ffn_w3, ffn_w1 = mega_ffn.split(n_dense, dim=0)
    return ffn_w1, ffn_w3
    ...
    with TemporaryDirectory() as tmp_model_path:
        ...
        for layer_i in range(n_layers):
            ...
            wq_proj, wk_proj, wv_proj = convert_wqkv(
                llama_mega=loaded, layer_idx=layer_i, n_heads=n_heads, n_heads_kv=n_heads_kv
            )
            ffn_w1, ffn_w3 = convert_ffn(llama_mega=loaded, layer_idx=layer_i, n_dense=n_dense)
            ...
            state_dict = {
                ...
                f"model.layers.{layer_i}.self_attn.o_proj.weight": loaded[
                    f"decoder.layers.{layer_i}.self_attention.linear_proj.weight"
                ],
                f"model.layers.{layer_i}.self_attn.q_norm.weight": loaded[
                    f"decoder.layers.{layer_i}.self_attention.q_layernorm.weight"
                ],
                f"model.layers.{layer_i}.self_attn.k_norm.weight": loaded[
                    f"decoder.layers.{layer_i}.self_attention.k_layernorm.weight"
                ],
                f"model.layers.{layer_i}.mlp.gate_proj.weight": ffn_w3,
                f"model.layers.{layer_i}.mlp.down_proj.weight": loaded[
                    f"decoder.layers.{layer_i}.mlp.linear_fc2.weight"
                ],
        ...
        state_dict = {
            ...
            "lm_head.weight": (
                ...
                if args1.untieembed
                else loaded["embedding.word_embeddings.weight"]
            ),
        ...
        ), f"config.rms_norm_eps: {config.rms_norm_eps}, norm_eps: {norm_eps} "
        ...
        model = AutoModelForCausalLM.from_pretrained(
            tmp_model_path, torch_dtype=torch_dtype, trust_remote_code=True
        )
...
def write_tokenizer(args: Namespace):
    ...
    if args.model in {"gptneox_1.7b_dlm", "gptneox_1.7b_ar"}:
        hf_tokenizer = AutoTokenizer.from_pretrained(
            os.path.join(PROJECT_DIR, f'megatron/training/tokenizer/gptneox_tokenizer')
        )
    elif args.model in {"gpt2_1b_dlm"}:
        hf_tokenizer = AutoTokenizer.from_pretrained(
            os.path.join(PROJECT_DIR, f'megatron/training/tokenizer/gpt2_tokenizer')
        )
    else:
...
def main():
    ...
    parser.add_argument("--input_dir", help="Location of Megatron weights", required=True)
    parser.add_argument("--num_output_shards", type=int, default=1)
    parser.add_argument(
        "--model",
        choices={"gptneox_1.7b_dlm", "gptneox_1.7b_ar", "gpt2_1b_dlm"},
    ...
    parser.add_argument("--rope_theta", type=float, default=1e4, help="Rope theta for the model")
    parser.add_argument(
        "--untieembed",
        action="store_true",
    ...
    args.model_dir_paths = {
        "gptneox_1.7b_dlm": os.path.join(
            PROJECT_DIR, f"tools/weights_conversion/hf_configs/gptneox_1.7b_dlm"
        ),
        "gptneox_1.7b_ar": os.path.join(
            PROJECT_DIR, f"tools/weights_conversion/hf_configs/gptneox_1.7b_ar"
        ),
        "gpt2_1b_dlm": os.path.join(
            PROJECT_DIR, f"tools/weights_conversion/hf_configs/gpt2_1b_dlm"
        ),
    }

    args.custom_models = ["gptneox_1.7b_dlm", "gptneox_1.7b_ar", "gpt2_1b_dlm"]

...
scale2emb = {'7B': 4096, '13B': 5120, '30B': 6656, '34B': 8192, '65B': 8192, '70B': 8192}
...
def merge_meta_llama(size: int, root_dir: Path):
    paths = sorted(
        path for path in root_dir.iterdir() if re.match(r"^consolidated\.[0-9]+\.pth$", path.name)
    )
    ...
    for i, ckpt_path in enumerate(tqdm(paths, desc="Merging llama")):
        ...
        if i == 0:
            merged_ckpt = init_merged_ckpt(
                llama_config, num_pth=num_pth, emb_dim=scale2emb[f"{size}B"]
            )
        else:
...
def merge_hf_llama(size: int, version: int, cache_dir: Optional[Path] = None):
    if version == 1:
        weights = LlamaForCausalLM.from_pretrained(
...
def merge_llama(size: int, version: int, root_dir: Optional[Path] = None):
    ...
    print(
        f"Weights at {root_dir} do not look like a meta checkpoint, assuming "

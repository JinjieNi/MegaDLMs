...
def permute_qkv_bias(
    bias_qkv: torch.Tensor, dim: int, n_heads: int, n_heads_kv: int, revert: bool = False
) -> torch.Tensor:
    def permute(x):
        head_dim = dim // n_heads
        if revert:
...
def update_checkpoint(input_dir: Path, output_dir: Path, overwrite_ok: bool = False):
    ...
    for fname in tqdm(list((input_dir / it).iterdir())):
        ...
        args = (args.hidden_size, args.num_attention_heads, args.num_attention_heads_kv)

...
from megatron.training.training import build_train_valid_test_data_iterators, setup_model_only
from pretrain_difflm import (
    extra_args_provider,
    get_batch,
    loss_func,
    model_provider,
...
def register_debug_hooks(model: torch.nn.Module, logfile: str):
    ...
    def hook_fn(module: torch.nn.Module, module_input, module_output):
        ...
        in_shapes = [
            inp.shape if isinstance(inp, torch.Tensor) else str(type(inp)) for inp in module_input
        ]
        in_slices = [
            slice_for_logging(inp) for inp in module_input if isinstance(inp, torch.Tensor)
        ]

        # Gather shapes and a small slice of output data
        out_shapes = [
            out.shape if isinstance(out, torch.Tensor) else str(type(out)) for out in module_output
        ]
        out_slices = [
            slice_for_logging(out) for out in module_output if isinstance(out, torch.Tensor)
        ]
...
def hf_forward(model, batch):
    ...
    output = model(
        input_ids=tokens, position_ids=position_ids, labels=tokens
    )  # tokens here as the labels should be right, as hf will deal with the label shiftin
...
def mega_forward(model, batch):
    ...
    loss_mask[:, -1] = (
        False  # in this megatron implementation, the input labels are shifted by 1, so we need to mask the last token
    )
...
def verify_step(activation_sample_path, our_forward, our_model, base_forward, base_model, batch):
    ...
    print(f"Abs loss error: {loss_error} " f"Our loss: {our_loss}, theirs: {base_loss}")
...
def extra_extra_args(parser):
    ...
    group.add_argument(
        "--activation-sample-path", type=str, help="Path to the activation sample file"
    )
    group.add_argument("--use-local-hf-model", action="store_true", help="Use local hf model")
    return parser
